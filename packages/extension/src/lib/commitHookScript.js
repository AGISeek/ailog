...
async function processCommit() {
    try {
        console.log('=== AIlog Post-Commit Processing ===');
        ...
        // 检查是否有AI归属标记
        const shouldAddAIAttribution = fs.existsSync(aiAttributionFlagPath);
        
        // 获取提交数据
        const commitData = {
            commit_time: new Date(latestCommit.date).getTime(),
            commit_hash: latestCommit.hash,
            repo: path.basename(workDir),
            branch: getCurrentBranch(),
            committer: latestCommit.author,
            is_ai_generated: shouldAddAIAttribution,
            code_volume_delta: getCodeVolumeDelta(latestCommit.hash),
            code_write_speed_delta: 0,
            notes: latestCommit.message
        };

        // 存储到数据库
        await saveToDatabase(commitData);
        console.log(`提交数据已保存: ${commitData.commit_hash}`);

        // 清理AI归属标记
        if (shouldAddAIAttribution) {
            fs.unlinkSync(aiAttributionFlagPath);
            console.log('AI归属标记已清理');
        }
    } catch (error) {
...
function getLatestCommit() {
    try {
        const output = execSync('git log -1 --format="%H|%an|%ae|%ad|%s"', { encoding: 'utf8' });
        const [hash, author, email, date, message] = output.trim().split('|');
 ...
 * 获取代码量变化
 */
function getCodeVolumeDelta(commitHash) {
    try {
        const output = execSync(`git show --stat --format="" ${commitHash}`, { encoding: 'utf8' });
        ...
        // 匹配 "X insertion(s)(+)" 格式
        const insertionMatch = output.match(/(\d+) insertions?\(\+\)/);
        if (insertionMatch) {
            insertions = parseInt(insertionMatch[1]) || 0;
        }
        
        // 匹配 "X deletion(s)(-)" 格式
        const deletionMatch = output.match(/(\d+) deletions?\(-\)/);
        if (deletionMatch) {
            deletions = parseInt(deletionMatch[1]) || 0;
        }
        
        return insertions - deletions;
    } catch (error) {
        console.error('获取代码量变化失败:', error);
        return 0;
    }
}
 ...
 * 保存到SQLite数据库
 */
async function saveToDatabase(commitData) {
    const sqlite3 = require('sqlite3').verbose();
    const dbPath = path.join(require('os').homedir(), '.watch-cursor', 'commits.db');
    ...
    return new Promise((resolve, reject) => {
        ...
        `, (err) => {
            ...
            db.run(sql, [
                ...
                commitData.is_ai_generated ? 1 : 0,
                commitData.code_volume_delta,
                commitData.code_write_speed_delta,
                commitData.notes
            ], function(err) {
                db.close();
                if (err) {
