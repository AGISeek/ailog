...
# 尝试通过VS Code命令调用
if command -v code >/dev/null 2>&1; then
    # 通过VS Code打开一个空文件并执行命令
    code --command "workbench.action.showCommands" --command "ailog.processCommit" 2>/dev/null &
    
    # 备用方案：通过Node.js直接调用
    node_script_path="$(dirname "$0")/../../src/lib/commitHookScript.js"
    if [ -f "$node_script_path" ]; then
        node "$node_script_path" process-commit 2>/dev/null &
    fi
else
    # 后备方案：使用Node.js脚本
    node_script_path="$(dirname "$0")/../../src/lib/commitHookScript.js"
    if [ -f "$node_script_path" ]; then
        node "$node_script_path" process-commit
    else
        echo "Warning: AI log processing unavailable"
        exit 0
    fi
fi
`;
